...
def sample_vector_field(grid_x: np.ndarray, grid_y: np.ndarray, grid_z: np.ndarray,
    ...
    # some hacky logic 
    function_chosen = divergence_free_kernel if kernel_func == 'divergence_free_kernel' else divergence_and_curl_free_kernel
...
def updated_vector_field(sampled_points_x: np.ndarray, sampled_points_y: np.ndarray, sampled_points_z: np.ndarray,
    ...
    # Flatten observations into single vector y
    y = np.zeros(M * D)
    for i in range(M):
        y[i*D:(i+1)*D] = fields_sampled[i]  # (3x,)

    # GP posterior mean and covariance
    K_inv = np.linalg.inv(K_x_x)
    posterior_mean = K_xstar_x @ K_inv @ y
    posterior_covar = K_xstar_xstar - K_xstar_x @ K_inv @ K_xstar_x.T
